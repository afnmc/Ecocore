...
public final class MinionTaskScheduler {
    ...
    private Iterable<UUID> distinctOwners() {
    ...
    private boolean performFishingAction(MinionInstance instance) {
        if (!instance.hasFreeStorageSlot()) return false;
        if (Math.random() < 0.35) {
    ...
    private boolean hasChestNearby(Location location) {
        ...
        for (BlockFace face : new BlockFace[]{BlockFace.NORTH, BlockFace.SOUTH, BlockFace.EAST, BlockFace.WEST}) {
            Block adjacent = centerBlock.getRelative(face);
