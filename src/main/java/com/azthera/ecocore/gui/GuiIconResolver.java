...
public final class GuiIconResolver {
    ...
    public ItemStack resolveIcon(YamlConfiguration guiConfig, String iconKey, String displayNameOverride) {
        ...
        if (displayNameOverride != null) {
            ...
            // FIX: Read name from 'names.' section in config if no override is provided
            String nameFromConfig = guiConfig.getString("names." + iconKey);
            if (nameFromConfig != null && !nameFromConfig.isBlank()) {
                builder.name(miniMessage.deserialize(nameFromConfig));
            }
        }
